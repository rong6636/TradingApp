        ...
        for orderId in j_order["twStocks"]:
            ...
            if gamerId in j_gamer:
                ...
                if orderId not in j_gamer[gamerId]["detail"][twtoday]:
                    j_gamer = newTwStocksDetail(order, datetime.now(timezone(timedelta(hours=+8))).strftime("%Y-%m-%d"), j_gamer, orderId, gamerId)
        w_gamer(j_gamer)
        j_order["twStocks"] = {}
...
def get_futures_info_i(futures, url):
    ...
    data = req.text
    # 當前交易量
    fv = int(data[data.find(':',data.find('"413":'))+1 :data.find(',', data.find('"413":'))])
    # 當前最大買價
    ...
    # 委買量 bid volume
    bvp = float(float(data[data.find(':',data.find('"109":'))+1 :data.find(',', data.find('"109":'))]))
    bv1 = float(data[data.find(':',data.find('"113":'))+1 :data.find(',', data.find('"113":'))])
    bv2 = float(data[data.find(':',data.find('"115":'))+1 :data.find(',', data.find('"115":'))])
    bv3 = float(data[data.find(':',data.find('"117":'))+1 :data.find(',', data.find('"117":'))])
    bv4 = float(data[data.find(':',data.find('"119":'))+1 :data.find(',', data.find('"119":'))])
    bv5 = float(data[data.find(':',data.find('"121":'))+1 :data.find(',', data.find('"121":'))])
    # 委賣量 ask volume
    svp = float(float(data[data.find(':',data.find('"110":'))+1 :data.find(',', data.find('"110":'))]))
    sv1 = float(data[data.find(':',data.find('"114":'))+1 :data.find(',', data.find('"114":'))])
    sv2 = float(data[data.find(':',data.find('"116":'))+1 :data.find(',', data.find('"116":'))])
    sv3 = float(data[data.find(':',data.find('"118":'))+1 :data.find(',', data.find('"118":'))])
    sv4 = float(data[data.find(':',data.find('"120":'))+1 :data.find(',', data.find('"120":'))])
    sv5 = float(data[data.find(':',data.find('"122":'))+1 :data.find(',', data.find('"122":'))])
    # point 單點價格
    point_list ={
        "大台指近一": 200,
        "大台指近二": 200,
    ...
    if "msgArray" in data:
        for info in data["msgArray"]:
            ...
            stocks_info[ticker] = {
                ...
                "price": float(info["z"]),
                "tv": int(info["tv"]),
                "best_ask_p": [float(i) for i in info["a"].split("_")[:-1]],
                "best_bid_p": [float(i) for i in info["b"].split("_")[:-1]],
                "best_ask_v": [float(i) for i in info["f"].split("_")[:-1]],
                "best_bid_v": [float(i) for i in info["g"].split("_")[:-1]]
            }
    else:
        print ("data", data)
...
# 受使用者委託
def user_entrust_order(user, market, type, ticker, name, price, lots, reqtime, extra):
    lots = int(lots)
    price = float(price)
    # 判斷能委託之時間
    dpart = get_currentTWSEDaysPart()[0]
    print ("get_currentTWSEDaysPart", dpart)
    order={}
    j_gamer = r_gamer()
    orderid = user + datetime.now().strftime("%M")+str(random.randint(100, 1000))
    initial_margin = get_tw_futures_initial_margin(ticker)
    available_money = get_available_money(user)
    ...
    if "不" not in orderid :
        ...
        while r_signal()["roundCD"] == 0:
            ...
            if count > 2:
                print("123")
                continue_order_thread = threading.Thread(target = thread_continue_order, args = (order, market, orderid,))
...
def thread_continue_order(order, market, orderid):
    count = 0
    while r_signal()["roundCD"] == 0:
        time.sleep(0.5)
        count+=1
...
def get_tw_futures_initial_margin(futures):
    # 造冊
    if "小台指" in futures:
        futures = "小型臺指"
...
def get_user_details(user, password):
    jgamer = r_gamer()
    if user not in jgamer:
        return "使用者不存在"
    if jgamer[user]["Password"] != password:
        return "密碼錯誤"
        
    detailList = {}
    if user in jgamer:
        for day in jgamer[user]["detail"]:
            detailList[day]=jgamer[user]["detail"][day].copy()
...
def get_user_account_details(user, password):
    ...
    detaiL={
        "Name": jgamer[user]["Name"],
        "SigninTimes": jgamer[user]["SigninTimes"],
        "lastSinginTime": jgamer[user]["lastSinginTime"],
    }
    return detaiL
...
def get_user_instock(user, password):
    ...
    portfolio={
        "twStocks": jgamer[user]["twStocks"],
        "twFutures": jgamer[user]["twFutures"]
    }
    ...
    if user not in j_gamer:
        ...
        if j_gamer[user]["Password"] != password:
            ...
            sa_json = {
                ...
                "unrealizedProfit": get_current_futures_income(user)
            }
            return sa_json
...
def get_current_futures_income(user):
    ...
    if user in j_gamer:
        for f in j_gamer[user]["twFutures"]:
            ...
            income += f_info[6]*(j_gamer[user]["twFutures"][f]["Lots"]*f_info[2] - j_gamer[user]["twFutures"][f]["Lots"]*j_gamer[user]["twFutures"][f]["Average_Price"])
...
def user_login(user, password):
    tw = timezone(timedelta(hours=+8))
    twtoday = datetime.now(tw).strftime("%Y-%m-%d %H:%M:%S")
    j_gamer = r_gamer()
    if user in j_gamer:
        if j_gamer[user]["Password"] == password:
            j_gamer[user]["SigninTimes"] += 1
            j_gamer[user]["lastSinginTime"] = twtoday
            return "Ok!"
        else:
            return "密碼錯誤!"
    else:
...
def r_log():
    ...
    filepath = "log/"+twdt.strftime("%d")+"/"+twdt.strftime("%H_%M")+".json"
    if not os.path.isdir("log/"+twdt.strftime("%d")):
        os.mkdir("log/"+twdt.strftime("%d"))
    if os.path.isfile(filepath) == False:
        log = {}
        with open (filepath, 'w', encoding='utf8') as f:
...
# renew GS_Log
def renew_Gs_Log():
    filepath = "GS_Log/"+datetime.now(timezone(timedelta(hours=+8))).strftime("%Y-%m-%d_%H")+".json"
    if os.path.isfile(filepath) == False:
        j_gamer = r_gamer()
        with open (filepath, 'w', encoding='utf8') as f:
