    ...
    for dId in j_order["twStocksDividend"]:
        ...
        if divi["lastState"] == "委":
            ...
            if gamerId in j_gamer:
                if twtoday not in j_gamer[gamerId]["detail"]:
                    j_gamer[gamerId]["detail"][twtoday] = {}
                rw.w_str_to_log("working_dividend() 除權息："+ gamerId + "," +divi["ticker"] + ","  + divi["type"] + "," +divi["a_p"] + "," +divi["totalLots"])
    ...
    if "msgArray" in data:
        for info in data["msgArray"]:
            ...
            stocks_info[ticker] = {
                "name": info["n"],
                "time": info["t"],
                "price": float(info["z"].replace("-", info["b"].split("_")[0])),
                "tv": int(info["tv"].replace("-", info["g"].split("_")[0])),
