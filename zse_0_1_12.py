...
def get_currentTWSEDaysPart():
    ...
    if int(twdt.strftime("%H%M"))>=800 and int(twdt.strftime("%H%M"))<900:
        _str += "股票開盤時段"
    # 不是例假日 不是六日 在9點到13點半之間
    if twdt.strftime("%Y/%m/%d") not in holiday["Holiday"]:
        if int(twdt.strftime("%w"))>=1 and int(twdt.strftime("%w"))<=5:
            if int(twdt.strftime("%H%M"))>=900 and int(twdt.strftime("%H%M"))<1330:
                _str += "股票交易時段"
            ...
            if order["state"] == "委":
                ...
                if available_money/order["price"] < order["new_lots"]:
                    ...
                    if order["price"] == info_p:
                        ...
                        o_lots-=m
                        p_v[info_p] = m
                        for i in range(len(s_info["best_bid_p"])):
        ...
        for orderId in j_order["twStocks"]:
            ...
            if gamerId in j_gamer:
                ...
                if orderId not in j_gamer[gamerId]["detail"][twtoday]:
                    j_gamer = newTwStocksDetail(order, j_gamer, twtoday, orderId, gamerId)
        j_order["twStocks"] = {}
        rw.w_gamer(j_gamer)

        # 判斷 明日除權息股 並預支付
        y_dividend = get_ex_dividend_stocks()
        print (y_dividend)
        for gamerId in j_gamer:
            if "Name" in j_gamer[gamerId]:
                for ticker in j_gamer[gamerId]["twStocks"]:
                    if ticker[4:] in y_dividend:
                        # 除權>>>給股票
                        if "權" in y_dividend[ticker[4:]]["除權息"]:
                            dividendOrder = {
                                "state": "委",
                                "time": datetime.now(tw).strftime("%m-%d %H:%M:%S"),
                                "type": "除權",
                                "ticker": ticker,
                                "name": y_dividend[ticker[4:]]["名稱"],
                                "totalLots": j_gamer[gamerId]["twStocks"][ticker]["Lots"],
                                "income": 0,
                                "detail": {},
                                "a_p": float(y_dividend[ticker[4:]]["無償配股率"])
                            }
                            j_order["twStocksDividend"][gamerId+"_D"+ticker[4:]] = dividendOrder
                            
                        # 除息>>>給錢
                        if "息" in y_dividend[ticker[4:]]["除權息"]:
                            dividendOrder = {
                                "state": "委",
                                "time": datetime.now(tw).strftime("%m-%d %H:%M:%S"),
                                "type": "除息",
                                ...
                                "name": y_dividend[ticker[4:]]["名稱"],
                                "totalLots": j_gamer[gamerId]["twStocks"][ticker]["Lots"],
                                "income": round(float(y_dividend[ticker[4:]]["現金股利"])*j_gamer[gamerId]["twStocks"][ticker]["Lots"]*1000, 2),
                                "detail": {},
                                "a_p": float(y_dividend[ticker[4:]]["現金股利"])
                            }
                            j_order["twStocksDividend"][gamerId+"_D"+ticker[4:]] = dividendOrder
...
# 處理 除權息
def working_dividend():
    j_gamer = rw.r_gamer()
    j_order = rw.r_order()
    twtoday =  datetime.now(timezone(timedelta(hours=+8))).strftime("%Y-%m-%d")

    for dId in j_order["twStocksDividend"]:
        print ("twStocksDividend：", dId)
        divi = j_order["twStocksDividend"][dId]
        if divi["state"] == "委":
            gamerId = dId[:-6]
            if gamerId in j_gamer:
                if twtoday not in j_gamer[gamerId]["detail"]:
                    j_gamer[gamerId]["detail"][twtoday] = {}
                print(divi)
                divi["detail"][datetime.now(timezone(timedelta(hours=+8))).strftime("%H:%M:%S.%f")] = divi["type"]+"。每股 "+ str(round(divi["a_p"], 2))
                
                if divi["type"] == "除息":
                    j_gamer[gamerId]["Cash"]+=divi["income"]
                    j_gamer[gamerId]["realizedProfit"]+=divi["income"]
                    divi["state"] = "成"

                if divi["type"] == "除權":
                    j_gamer[gamerId]["twStocks"][divi["ticker"]]["Lots"] += int(divi["a_p"]*divi["totalLots"])
                    divi["state"] = "成"
                j_gamer[gamerId]["detail"][twtoday][dId] = divi
                rw.w_str_to_log("working_dividend() 除權息："+ gamerId + "," +divi["ticker"] + ","  + divi["type"] + "," + str(divi["a_p"]) + "," + str(divi["totalLots"]))
        ...
        for gamerId in j_gamer:
            if "Name" in j_gamer[gamerId]:
                ...
                for twF in j_gamer[gamerId]["twFutures"]:
                    maintenance+=j_gamer[gamerId]["twFutures"][twF]["Lots"]*get_twFutures_maintenance_margin(twF)
                # === 需要強制平倉 ===
...
def newTwStocksDetail(order, j_gamer, twtoday, orderId, gamerId):
    ...
    if content["totalLots"] > 0:
        for i in order["p_v"]:
            ...
            count_v += order["p_v"][i]
        content["a_p"] = round(count_p/count_v, 2)
        print(twtoday)
        if twtoday not in j_gamer[gamerId]["detail"]:
            j_gamer[gamerId]["detail"][twtoday] = {}
...
def get_stocks_info(str_stocks):
    ...
    if "msgArray" in data:
        ...
        for info in data["msgArray"]:
            ...
            if info["a"] == "-":
                start = 0
                if float(info["b"].split("_")[:-1][0]) == 0:
                    start = 1
                stocks_info[ticker]["price"] = float(info["z"].replace("-", info["b"].split("_")[start]))
                stocks_info[ticker]["tv"] = int(info["tv"].replace("-", info["g"].split("_")[0]))
                stocks_info[ticker]["best_ask_p"] = [float((info["b"].split("_")[:-1])[start])]
                stocks_info[ticker]["best_bid_p"] = [float(i) for i in info["b"].split("_")[:-1]][start:]
                stocks_info[ticker]["best_ask_v"] = [0]
                stocks_info[ticker]["best_bid_v"] = [float(i) for i in info["g"].split("_")[:-1]][start:]
            elif info["b"] == "-":
                start = 0
                if float(info["a"].split("_")[:-1][0]) == 0:
                    start = 1
                stocks_info[ticker]["price"] = float(info["z"].replace("-", info["a"].split("_")[start]))
                stocks_info[ticker]["tv"] = int(info["tv"].replace("-", info["f"].split("_")[0]))
                stocks_info[ticker]["best_ask_p"] = [float(i) for i in info["a"].split("_")[:-1]][start:]
                stocks_info[ticker]["best_bid_p"] = [float((info["a"].split("_")[:-1])[start])]
                stocks_info[ticker]["best_ask_v"] = [float(i) for i in info["f"].split("_")[:-1]][start:]
                stocks_info[ticker]["best_bid_v"] = [0]
            else:
...
# 更新 除權息報告
def renew_dividend_report(j_dividend, twdt):
    if j_dividend["renewTime"] >= twdt:
        return 
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.76 Safari/537.36'}
    url = "https://www.twse.com.tw/exchangeReport/TWT48U?response=josn"
    req = requests.get(url, headers= headers)
    reqData = json.loads(req.text)
    column = reqData["fields"]

    data = {
        "BackupTime":j_dividend["BackupTime"],
        "renewTime":twdt,
        "ex_dividend_day": twdt
    }
    for d in range(len(reqData["data"])):
        r = reqData["data"][d]
        date = str(int(r[0][:3])+1911)+"/"+r[0][4:6]+"/"+r[0][7:9]
        if date not in data:
            data[date] = {}
        ticker = r[1]
        data[date][ticker] = {}
        for i in range(len(column)):
            data[date][ticker][column[i]] = r[i]
    time.sleep(10)
    rw.w_dividend(data)

# 取得明日除權息股
def get_ex_dividend_stocks():
    j_dividend = rw.r_dividend()
    twdt = (datetime.now(timezone(timedelta(hours=+8)))+timedelta(days=1)).strftime("%Y/%m/%d")
    renew_thread = threading.Thread(target=renew_dividend_report, args=(j_dividend, twdt,))
    renew_thread.start()
    if twdt in j_dividend and j_dividend["ex_dividend_day"] < twdt:
        j_dividend["ex_dividend_day"] = twdt
        rw.w_dividend(j_dividend)
        return j_dividend[twdt]
    return {}
...
def get_user_order(user):
    ...
    for d in jdata["twStocksDividend"]:
        if user in d:
            orderlist[d]=jdata["twStocksDividend"][d].copy()
